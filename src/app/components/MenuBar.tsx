...
export function MenuBar({ open, onClose }: MenuBarProps) {
  ...
  // Determine if user is a tenant (has rented house but not an admin)
  const isTenant =
    isAuthenticated &&
    !user?.isSuperAdmin &&
    !user?.isAdmin &&
                ...
                {isAuthenticated && user ? (
                  <>
                    ...
                    <button
                      ...
                      className="flex items-center gap-3 px-3 py-2.5 rounded-xl w-full transition-colors hover:bg-red-500/10"
                      style={{ color: "#ef4444" }}
                    >
                      <LogOut size={16} />
